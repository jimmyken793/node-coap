...
CoAPServer.prototype._handle = function(packet, rsinfo) {
  ...
  response = new Message(packet, function(response, responsePacket) {
    var buf
      , sender = new RetrySend(sock, rsinfo.port, rsinfo.address)

    try {
      buf = generate(responsePacket)
    } catch(err) {
    ...
    var key = toKey(rsinfo.address, rsinfo.port,
        responsePacket, responsePacket.ack || !responsePacket.confirmable)
    lru.set(key, buf)
    buf.sender = sender

    if (that._options.sendAcksForNonConfirmablePackets || packet.confirmable){
      sender.send(buf, responsePacket.ack || responsePacket.reset || responsePacket.confirmable === false)
    } else {
...
OutMessage.prototype.end= function(payload) {
  ...
  if (payload.length % requestedBlockOption.size === 0)
